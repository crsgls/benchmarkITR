# Compare ITR of different meta learner on the dataset "ist"
...
#incorporating natural cubic spline for AGE and RSBP
fit <- lrm(deathdep ~ (SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region)*aspirin, data=train.sl)
...
pred0.test <- predict(fit, newdata=test0, type="fitted")
pred1.test <- predict(fit, newdata=test1, type="fitted")

test.sl$ITE <- pred1.test-pred0.test
test.sl$ITR <- ifelse(test.sl$ITE<0,1,0)

mean(test.sl$ITR) #ITR=1 => 100%

test.eq.sl <- test.sl[test.sl$aspirin == test.sl$ITR, ]
v.test.sl <- mean(test.eq.sl$deathdep)  #0.65
...
pred0.train <- predict(fit, newdata=train0, type="fitted")
pred1.train <- predict(fit, newdata=train1, type="fitted")

train.sl$ITE <- pred1.train-pred0.train
train.sl$ITR <- ifelse(train.sl$ITE<0,1,0)

mean(train.sl$ITR) #ITR=1 => 100%

train.eq.sl <- train.sl[train.sl$aspirin==0 & train.sl$ITR==0 | train.sl$aspirin==1 & train.sl$ITR==1,]
v.train.sl <- mean(train.eq.sl$deathdep)  #0.61
...
fit0 <- lrm(deathdep ~ SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region,
            data = train.tl,
            subset = aspirin == 0)
fit1 <- lrm(deathdep ~ SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region,
            data = train.tl,
            subset = aspirin == 1)

#ITR w/ test
pred0.test <- predict(fit0, newdata=test.tl, type="fitted")
pred1.test <- predict(fit1, newdata =test.tl, type="fitted")

test.tl$ITE <- pred1.test-pred0.test
test.tl$ITR <- ifelse(test.tl$ITE<0,1,0)

mean(test.tl$ITR) #ITR=1 => 63%

test.eq.tl <- test.tl[test.tl$aspirin == test.tl$ITR, ]
v.test.tl <- mean(test.eq.tl$deathdep)  #0.65

#ITR w/ train
pred0.train <- predict(fit0, newdata=train.tl, type="fitted")
pred1.train <- predict(fit1, newdata =train.tl, type="fitted")

train.tl$ITE <- pred1.train-pred0.train
train.tl$ITR <- ifelse(train.tl$ITE<0,1,0)

mean(train.tl$ITR) #ITR=1 => 62.7%

train.eq.tl <- train.tl[train.tl$aspirin == train.tl$ITR, ]
v.train.tl <- mean(train.eq.tl$deathdep)  #0.59
...
fit0 <- lrm(deathdep ~ SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region, data=train.xl, subset=aspirin==0)
fit1 <- lrm(deathdep ~ SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region, data=train.xl, subset=aspirin==1)

df0 <- filter(train.xl, aspirin == 0)
df1 <- filter(train.xl, aspirin == 1)
mu0 <- predict(fit0, newdata=df1, type="fitted")
mu1 <- predict(fit1, newdata=df0, type="fitted")
...
tau0 <- lm(D0~SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region, data=df0)
tau1 <- lm(D1~SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region, data=df1)

#ITR w/ test
pred0.test <- predict(tau0, newdata=test.xl, type="response")
pred1.test <- predict(tau1, newdata=test.xl, type="response")

#Calculate g via propensity score
mod_ps <- glm(aspirin ~ SEX + rcs(AGE, c(57, 74, 85)) + rcs(SBP, c(130, 160,200)) + RDELAY + CTbeforeHosp + CTInfarct + AtrialFib + Asp3d + FaceDef + ArmHandDef + LegFootDef + Dysphasia + Hemianopia + VSDisorder + CerebSigns + OtherDeficit + Conscious + StrokeType + region,
              family = binomial,
              data = train.xl)
pr_score <- predict(mod_ps, type = "response")
mean(pr_score) #we choose g=0.5
g <- 0.5 #weighting function between 0 & 1

test.xl$ITE <- g*pred0.test + (1-g)*pred1.test
test.xl$ITR <- ifelse(test.xl$ITE<0,1,0)

mean(test.xl$ITR) #ITR=1 => 67.1% 

test.eq.xl <- test.xl[test.xl$aspirin == test.xl$ITR, ]
v.test.xl <- mean(test.eq.xl$deathdep)  #0.65

#ITR w/ train
pred0.train <- predict(tau0, newdata=train.xl, type="response")
pred1.train <- predict(tau1, newdata=train.xl, type="response")

train.xl$ITE <- pred1.train-pred0.train
train.xl$ITR <- ifelse(train.xl$ITE<0,1,0)

mean(train.xl$ITR) #ITR=1 => 59.2%

train.eq.xl <- train.xl[train.xl$aspirin == train.xl$ITR, ]
v.train.xl <- mean(train.eq.xl$deathdep)  #0.61
...
heatmap(test, test.tl$ITR, test.xl$ITR, covariates)
...
ite_scatter_plot(ite1 = test.xl$ITE,
                 ite2 = test.tl$ITE,
                 label = c("X learner", "T learner"))

ite_scatter_plot(ite1 = test.sl$ITE,
                 ite2 = test.tl$ITE,
